...
export default function DetalhesD3_00034({ detalhes }: { detalhes: any[] }) {
    ...
    <Table>
      <TableHeader>
        <TableRow className="bg-[#e3eef6]/50 hover:bg-[#e3eef6]/50">
          ...
          <TableHead className="text-xs font-semibold text-[#033e66] text-right">Outro Anexo</TableHead>
      ...
      <TableBody>
        {detalhes.map((d: any, i: number) => (
          <TableRow key={i} className={!d.consistente ? "bg-[#ef444408] hover:bg-[#ef444412]" : "hover:bg-[#e3eef6]/30"}>
            ...
            <TableCell className="text-xs font-semibold text-[#045ba3] whitespace-nowrap align-top">{d.comparacao}</TableCell>
            <TableCell className="text-xs text-[#6b7280] align-top">
              ...
              <div className="leading-5 mt-0.5">
                <span className="font-mono text-[#033e66]">{d.outro_cod_conta}</span>
                <span className="text-[#6b7280]"> | {d.col_outro}</span>
                <span className="text-[#6b7280]"> ({d.outro_anexo.replace("RREO-", "")})</span>
              </div>
            </TableCell>
            <TableCell className="text-sm text-right font-mono text-[#045ba3] align-top">{fmt(d.an01_valor)}</TableCell>
            <TableCell className="text-sm text-right font-mono text-[#045ba3] align-top">{fmt(d.outro_valor)}</TableCell>

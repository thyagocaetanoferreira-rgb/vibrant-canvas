-- =============================================================
-- TCM-GO: fn_processar_remessa v3
-- Adiciona blocos ETL ausentes do arquivo AOC:
--   16b. AOC.11 → fato_alteracao_orcamentaria_unidade
--   16c. AOC.12 → fato_alteracao_orcamentaria_fonte
--   16d. AOC.90 → fato_aoc_ato (subtipo '90')
--   16e. AOC.91 → fato_aoc_ato (subtipo '91')
--   16f. AOC.92 → fato_aoc_ato (subtipo '92')
--   16g. AOC.93 → fato_aoc_ato (subtipo '93')
--   16h. AOC.94 → fato_aoc_ato (subtipo '94')
-- Total: 30 tabelas stg_* → fato_*
-- =============================================================
...
BEGIN
    ...
    VALUES (
        p_remessa_id,
        'etl_concluido',
        'Pipeline stg_* → fato_* (v3) concluído. 30 tabelas processadas (AOC completo).'
    );

    RAISE NOTICE 'fn_processar_remessa(%) v3 concluído.', p_remessa_id;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION fn_processar_remessa(INTEGER) IS
    'ETL v3: transforma stg_* → fato_* (30 tabelas). '
    'AOC completo: layouts 10, 11, 12, 90, 91, 92, 93, 94. '
    'Compatível com schema 2026-03-24.';

-- =============================================================
-- Migração: DIC ETL v4 — DIC.10 → fato_divida_consolidada
-- Data: 2026-03-24
-- Descrição: Adiciona bloco ETL da Dívida Consolidada (DIC.10)
--            na função fn_processar_remessa e reprocessa remessas
--            que já têm dados em stg_dic_10.
-- =============================================================

CREATE OR REPLACE FUNCTION public.fn_processar_remessa(p_remessa_id integer)
 RETURNS TABLE(tabela character varying, linhas_inseridas integer)
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- ==========================================================
    -- 16b. AOC.11 → fato_alteracao_orcamentaria_unidade
    -- Det. Alterações Orçamentárias por Elemento de Despesa
    -- ==========================================================
    INSERT INTO fato_alteracao_orcamentaria_unidade (
        municipio_id, orgao_id, ano_referencia, mes_referencia, remessa_id,
        cod_programa, cod_orgao, cod_unidade, cod_funcao, cod_subfuncao,
        natureza_acao, nro_proj_ativ,
        cod_natureza_despesa, dt_alteracao, nr_alteracao, tipo_alteracao,
        vl_alteracao, vl_saldo_ant_dotacao, vl_saldo_atual
    )
    SELECT
        v_municipio_id, v_orgao_id, v_ano, v_mes, p_remessa_id,
        TRIM(s.cod_programa), TRIM(s.cod_orgao), TRIM(s.cod_unidade),
        TRIM(s.cod_funcao), TRIM(s.cod_subfuncao),
        TRIM(s.natureza_acao), TRIM(s.nro_proj_ativ),
        TRIM(s.cod_natureza_despesa),
        fn_parse_data(s.dt_alteracao),
        TRIM(s.nr_alteracao),
        TRIM(s.tipo_alteracao),
        fn_parse_valor(s.vl_alteracao),
        fn_parse_valor(s.vl_saldo_ant_dotacao),
        fn_parse_valor(s.vl_saldo_atual)
    FROM stg_aoc_11 s
    WHERE s.remessa_id = p_remessa_id;
    GET DIAGNOSTICS v_count = ROW_COUNT;
    tabela := 'fato_alteracao_orcamentaria_unidade'; linhas_inseridas := v_count; RETURN NEXT;

    -- ==========================================================
    -- 16c. AOC.12 → fato_alteracao_orcamentaria_fonte
    -- Det. Alterações Orçamentárias por Fonte de Recurso
    -- ==========================================================
    INSERT INTO fato_alteracao_orcamentaria_fonte (
        municipio_id, orgao_id, ano_referencia, mes_referencia, remessa_id,
        cod_programa, cod_orgao, cod_unidade, cod_funcao, cod_subfuncao,
        natureza_acao, nro_proj_ativ,
        cod_natureza_despesa, dt_alteracao, nr_alteracao, tipo_alteracao,
        cod_fonte_recurso, vl_alteracao_fonte, vl_saldo_ant_fonte, vl_saldo_atual_fonte
    )
    SELECT
        v_municipio_id, v_orgao_id, v_ano, v_mes, p_remessa_id,
        TRIM(s.cod_programa), TRIM(s.cod_orgao), TRIM(s.cod_unidade),
        ...
        fn_parse_valor(s.vl_alteracao_fonte),
        fn_parse_valor(s.vl_saldo_ant_fonte),
        fn_parse_valor(s.vl_saldo_atual_fonte)
    FROM stg_aoc_12 s
    WHERE s.remessa_id = p_remessa_id;
    GET DIAGNOSTICS v_count = ROW_COUNT;
    tabela := 'fato_alteracao_orcamentaria_fonte'; linhas_inseridas := v_count; RETURN NEXT;

    -- ==========================================================
    -- 16d. AOC.90 → fato_aoc_ato (Lei de Suplementação)
    -- ==========================================================
    INSERT INTO fato_aoc_ato (
        municipio_id, orgao_id, ano_referencia, mes_referencia, remessa_id,
        subtipo, nr_lei_suplementacao, data_lei_suplementacao, vl_autorizado
    )
    SELECT
        v_municipio_id, v_orgao_id, v_ano, v_mes, p_remessa_id,
        '90',
        TRIM(s.nr_lei_suplementacao),
        fn_parse_data(s.data_lei_suplementacao),
        fn_parse_valor(s.vl_autorizado)
    FROM stg_aoc_90 s
    WHERE s.remessa_id = p_remessa_id;
    GET DIAGNOSTICS v_count = ROW_COUNT;
    tabela := 'fato_aoc_ato[90]'; linhas_inseridas := v_count; RETURN NEXT;

    -- ==========================================================
    -- 16e. AOC.91 → fato_aoc_ato (Lei de Crédito Especial)
    -- ==========================================================
    INSERT INTO fato_aoc_ato (
        municipio_id, orgao_id, ano_referencia, mes_referencia, remessa_id,
        subtipo, nr_lei_credito_esp, data_lei_credito_esp, vl_autorizado
    )
    SELECT
        v_municipio_id, v_orgao_id, v_ano, v_mes, p_remessa_id,
        '91',
        TRIM(s.nr_lei_credito_esp),
        fn_parse_data(s.data_lei_credito_esp),
        fn_parse_valor(s.vl_autorizado)
    FROM stg_aoc_91 s
    WHERE s.remessa_id = p_remessa_id;
    GET DIAGNOSTICS v_count = ROW_COUNT;
    tabela := 'fato_aoc_ato[91]'; linhas_inseridas := v_count; RETURN NEXT;

    -- ==========================================================
    -- 16f. AOC.92 → fato_aoc_ato (Lei para Realocação de Recursos)
    -- ==========================================================
    INSERT INTO fato_aoc_ato (
        municipio_id, orgao_id, ano_referencia, mes_referencia, remessa_id,
        subtipo, nr_lei_realoc, data_lei_realoc, vl_autorizado
    )
    SELECT
        v_municipio_id, v_orgao_id, v_ano, v_mes, p_remessa_id,
        '92',
        TRIM(s.nr_lei_realoc),
        fn_parse_data(s.data_lei_realoc),
        fn_parse_valor(s.vl_autorizado)
    FROM stg_aoc_92 s
    WHERE s.remessa_id = p_remessa_id;
    GET DIAGNOSTICS v_count = ROW_COUNT;
    tabela := 'fato_aoc_ato[92]'; linhas_inseridas := v_count; RETURN NEXT;

    -- ==========================================================
    -- 16g. AOC.93 → fato_aoc_ato (Lei de Alteração do PPA)
    -- ==========================================================
    INSERT INTO fato_aoc_ato (
        municipio_id, orgao_id, ano_referencia, mes_referencia, remessa_id,
        subtipo, nr_lei_alt_ppa, data_lei_alt_ppa
    )
    SELECT
        v_municipio_id, v_orgao_id, v_ano, v_mes, p_remessa_id,
        '93',
        TRIM(s.nr_lei_alt_ppa),
        fn_parse_data(s.data_lei_alt_ppa)
    FROM stg_aoc_93 s
    WHERE s.remessa_id = p_remessa_id;
    GET DIAGNOSTICS v_count = ROW_COUNT;
    tabela := 'fato_aoc_ato[93]'; linhas_inseridas := v_count; RETURN NEXT;

    -- ==========================================================
    -- 16h. AOC.94 → fato_aoc_ato (Decreto de Abertura de Créditos)
    -- ==========================================================
    INSERT INTO fato_aoc_ato (
        municipio_id, orgao_id, ano_referencia, mes_referencia, remessa_id,
        subtipo, nr_decreto, data_decreto, vl_decreto, tipo_credito
    )
    SELECT
        v_municipio_id, v_orgao_id, v_ano, v_mes, p_remessa_id,
        '94',
        TRIM(s.nr_decreto),
        fn_parse_data(s.data_decreto),
        fn_parse_valor(s.vl_decreto),
        TRIM(s.tipo_credito)
    FROM stg_aoc_94 s
    WHERE s.remessa_id = p_remessa_id;
    GET DIAGNOSTICS v_count = ROW_COUNT;
    tabela := 'fato_aoc_ato[94]'; linhas_inseridas := v_count; RETURN NEXT;
    ...
    -- ==========================================================
    -- 24. DÍVIDA CONSOLIDADA (DIC.10 → fato_divida_consolidada)
    -- ==========================================================
    INSERT INTO fato_divida_consolidada (
        municipio_id, orgao_id, ano_referencia, mes_referencia, remessa_id,
        cod_orgao, cod_unidade,
        tp_lancamento, nro_lei_autorizacao, dt_lei_autorizacao,
        nome_credor, tipo_pessoa, cpf_cnpj_credor,
        vl_saldo_anterior, vl_contratacao, vl_amortizacao,
        vl_cancelamento, vl_encampacao, vl_atualizacao, vl_saldo_atual
    )
    SELECT
        v_municipio_id, v_orgao_id, v_ano, v_mes, p_remessa_id,
        TRIM(s.cod_orgao), TRIM(s.cod_unidade),
        TRIM(s.tp_lancamento), TRIM(s.nro_lei_autorizacao), fn_parse_data(s.dt_lei_autorizacao),
        TRIM(s.nome_credor), TRIM(s.tipo_pessoa), TRIM(s.cpf_cnpj_credor),
        fn_parse_valor(s.vl_saldo_anterior), fn_parse_valor(s.vl_contratacao), fn_parse_valor(s.vl_amortizacao),
        fn_parse_valor(s.vl_cancelamento), fn_parse_valor(s.vl_encampacao), fn_parse_valor(s.vl_atualizacao),
        fn_parse_valor(s.vl_saldo_atual)
    FROM stg_dic_10 s
    WHERE s.remessa_id = p_remessa_id;
    GET DIAGNOSTICS v_count = ROW_COUNT;
    tabela := 'fato_divida_consolidada'; linhas_inseridas := v_count; RETURN NEXT;
    ...
    VALUES (
        p_remessa_id,
        'etl_concluido',
        'Pipeline stg_* → fato_* (v4) concluído. 31 tabelas processadas (DIC adicionado).'
    );

    RAISE NOTICE 'fn_processar_remessa(%) v4 concluído.', p_remessa_id;
END;
$function$;

-- =============================================================
-- Reprocessar remessas que já têm dados em stg_dic_10
-- =============================================================
DO $$
DECLARE
...
BEGIN
    FOR v_remessa_id IN
        SELECT DISTINCT remessa_id FROM stg_dic_10 ORDER BY remessa_id
    LOOP
        -- Deleta eventuais dados antigos (segurança)
        DELETE FROM fato_divida_consolidada WHERE remessa_id = v_remessa_id;
        ...
        SELECT
            r.municipio_id, r.orgao_id, r.ano_referencia, r.mes_referencia, r.id,
            TRIM(s.cod_orgao), TRIM(s.cod_unidade),
            TRIM(s.tp_lancamento), TRIM(s.nro_lei_autorizacao), fn_parse_data(s.dt_lei_autorizacao),
            TRIM(s.nome_credor), TRIM(s.tipo_pessoa), TRIM(s.cpf_cnpj_credor),
            fn_parse_valor(s.vl_saldo_anterior), fn_parse_valor(s.vl_contratacao), fn_parse_valor(s.vl_amortizacao),
            ...
            fn_parse_valor(s.vl_saldo_atual)
        FROM stg_dic_10 s
        JOIN tcmgo_remessa r ON r.id = s.remessa_id
        WHERE s.remessa_id = v_remessa_id;

        GET DIAGNOSTICS v_count = ROW_COUNT;
        RAISE NOTICE 'Remessa %: % linhas inseridas em fato_divida_consolidada.', v_remessa_id, v_count;

        INSERT INTO tcmgo_historico_processamento (remessa_id, evento, descricao)
        VALUES (v_remessa_id, 'dic_reprocessado',
                'DIC.10 → fato_divida_consolidada reprocessado (migração v4). ' || v_count || ' linhas.');
    END LOOP;
END;
$$;

-- Verificação final
SELECT 'fato_divida_consolidada' AS tabela, COUNT(*) AS total_linhas FROM fato_divida_consolidada;
